...
import (
	...
	"github.com/mavryk-network/mavbingo/v2/protocol/core/expression"
...
func TestImport(t *testing.T) {
	...
	require.Equal(t, "mv1949pcbqwGsHfUCaVmNVRu21Cd4SnbpvpP", imported.Hash.String())

	list, err := s.ListPublicKeys(context.Background())
	require.NoError(t, err)
	require.Equal(t, []*mavsign.PublicKey{imported}, list)
}
...
func TestPolicy(t *testing.T) {
	type testCase struct {
		...
		policy   mavsign.PublicKeyPolicy
		expected string
	}
				...
				Contents: []latest.OperationContents{
					&latest.Transaction{
						ManagerOperation: latest.ManagerOperation{
							Source:       &mv.Ed25519PublicKeyHash{1, 2, 3},
							Fee:          mv.BigUint{0x00},
							Counter:      mv.BigUint{0x00},
							GasLimit:     mv.BigUint{0x00},
							StorageLimit: mv.BigUint{0x00},
						},
						Amount:      mv.BigUint{0x00},
						Destination: core.ImplicitContract{PublicKeyHash: &mv.Ed25519PublicKeyHash{1, 2, 3}},
						Parameters: mv.Some(latest.Parameters{
	...
	for _, c := range cases {
		t.Run(c.title, func(t *testing.T) {
			conf := mavsign.Config{
				...
				Policy: hashmap.NewPublicKeyHashMap([]hashmap.PublicKeyKV[*mavsign.PublicKeyPolicy]{{Key: pk.Hash(), Val: &c.policy}}),
			}
			...
			_, err = s.Sign(context.Background(), &mavsign.SignRequest{PublicKeyHash: pk.Hash(), Message: msg})
			if c.expected != "" {
